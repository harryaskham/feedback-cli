...
- `bd-f6015c` — [recovery] republish this summary after it was lost to the deferred
  state-publish / post-rebase reset race (see postscript)
...
- Code/content commit: `eb7717e`, squash-landed on `main` as `14a411f`.
...
the same trap within an hour, which is the real argument for the comment.

The second takeaway is the dark CI: feedback-cli accepted two unverified lands while every
run sat in a 24h queue timeout, and nothing anywhere raised a flag. A never-scheduled CI is
silent in a way a red CI never is; it reads as "no news". `main` turned out to be green,
which is luck rather than process. I filed the fleet-wide detection shape as a cacophony
draft, and handed `md4-0` a verified-green baseline so the first run after their runner
relabel was an unconfounded signal.

## Postscript — this summary was lost once, and recovered

The artefact you are reading did not publish on the first attempt. The `bd-221864`
reintegration landed `14a411f` and reported `1 artefact pending publication to
cacophony-state ... deferred to the async state-publish drain`. The same receipt told me to
run `caco agent rebase`, so I did — and its `bd-e65f72` step reset the state-only
`.cacophony/agent` commit that the pending drain still needed. An hour later
`caco summaries list` was empty, the `cacophony-state` branch had no path for this agent
(other agents on this project had theirs, so the pipeline itself was fine), and the local
file was gone. Following the receipt's own instruction destroyed the summary, silently.

It was recoverable from the backup ref the receipt itself names:
`git checkout refs/caco-backup/<agent>/<attempt-id>/head -- .cacophony/agent/<agent>/summary/pending/summary.md`
restored it intact. The daemon-side race is filed in cacophony as a P1; this republication
is `bd-f6015c`.

So this repo produced **two distinct silent summary-loss paths in one day** — gitignoring
`.cacophony/` (`bd-f08617`) and this deferred-publish race. Both fail identically: accepted,
no error, nothing published, and the only symptom is an empty `caco summaries list` that
nothing prompts anyone to check. If you are reading this after a recreate and it is the
only summary present, check whether it should have had company.
